...
const TABLE_NAME = 'it_prisma_adapter';
const dbConfig = getTestPostgresConfig();

class PgPrismaLikeClient implements PrismaRawExecutor {
  ...
  async $transaction<T>(cb: (tx: PrismaRawExecutor) => Promise<T>): Promise<T> {
    const client = await this.pool.connect();
    const txExecutor = this.createTxExecutor(client);
  ...
  private createTxExecutor(client: PoolClient): PrismaRawExecutor {
    return {
...
describe('Integration: PrismaWorkflowAdapter (real Postgres)', () => {
  let setupClient: Client;
  let pool: Pool;
  let adapter: PrismaWorkflowAdapter;
  ...
  beforeAll(async () => {
    ...
    const executor = new PgPrismaLikeClient(pool);
    adapter = new PrismaWorkflowAdapter(executor, TABLE_NAME);
  });
